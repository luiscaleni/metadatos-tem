...
export default function Memoji(){
    ...
    class BasicScene {
        constructor() {
            ...
            this.camera = new THREE.PerspectiveCamera(60, this.width / this.height, 0.01, 500);
            ...
            black.style.width="360";
            black.style.height="360";
    ...
    function detectFaceLandmarks(time) {
        scene.scene.position.x=-3;
        console.log(scene.scene.position.x);
        
        if (!faceLandmarker) {
            return;
        }
        const landmarks = faceLandmarker.detectForVideo(videoDom, time);
        ...
        if (transformationMatrices && transformationMatrices.length > 0) {
            ...
            avatar.applyMatrix(matrix, { scale: 13 }); 
        }
        // Apply Blendshapes
    ...
    function onVideoFrame(time) {
        ...
        videoDom.requestVideoFrameCallback(onVideoFrame);
    }
    ...
    async function streamWebcamThroughFaceLandmarker() {
        function onAcquiredUserMedia(stream) {
            ...
            videoDom.onloadedmetadata = () => {
                videoDom.play();
                value = videoDom.srcObject.getTracks()[0];
            };
        }
        try {
